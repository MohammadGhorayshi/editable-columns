...
<span
    ...
    @includeWhen(!empty($column['wrapper']), 'crud::columns.inc.wrapper_start')
        <div style="display: grid; grid-template-columns: fit-content(0%) 1fr;">
            {{-- Switch --}}
            <label class="switch switch-sm switch-label switch-pill switch-{{ $column['color'] }}" style="margin-bottom: 0;">
                <input
                    type="checkbox"
                    name="{{ $column['name'] }}"
                ...
                <span 
                    ...
                    tabindex="0"
                    onfocus="registerMinorEditInputEvents(this.previousElementSibling, event)"></span>
            </label>
            {{-- Status --}}
            <span class="status" style="width: 10px; height: 10px; display: inline-block; border-radius: 50%; opacity: 0.8; margin: 6px;"></span>
            {{-- Underline --}}
